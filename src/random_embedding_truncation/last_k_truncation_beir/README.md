# Last K% Truncation for Retrieval

This directory is in charge of *Last K% Truncation* (Sec. 2 in the paper) with retrieval datasets.
...
```sh
uv run  \
  python ./src/random_embedding_truncation/last_k_truncation_beir/main.py \
  --config ./src/random_embedding_truncation/last_k_truncation_beir/configs/sentence-t5/sentence-t5-arguana.toml
```
