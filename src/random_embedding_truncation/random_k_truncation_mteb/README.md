# Random K% Truncation for Classification

This directory is in charge of *Random K% Truncation* (Sec. 2 in the paper) with classification datasets.
...
```sh
uv run  \
  python ./src/random_embedding_truncation/random_k_truncation_mteb/main.py \
  --config ./src/random_embedding_truncation/random_k_truncation_mteb/configs/all-mpnet-base-v2/all-mpnet-base-v2.toml
```
